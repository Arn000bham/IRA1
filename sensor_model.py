...
class SensorModel(object):
    def __init__(self):
        ...
        self.z_short = 0.1  # Probability of a short reading from an unexpected obstacle (e.g. person or object)
        self.z_max = 0.05  # Probability of failure to detect an obstacle, reported as max range
        self.z_rand = 0.05  # Random noise on all readings

        self.sigma_hit = 0.2  # Noise on hit
        self.lambda_short = 0.1  # Noise on short reading

        # Initialize scan parameters to nothing
    ...
    def set_laser_scan_parameters(self, num_readings, scan_range_max,
                                  ...
                                  scan_angle_max):
        # Set the parameters for the laser scanner that this instance is modeling
    ...
    def set_map(self, occupancy_map):
        # Set the map that this model should use when calculating expected laser readings
        self.map_width = occupancy_map.info.width
        self.map_height = occupancy_map.info.height
        self.map_resolution = occupancy_map.info.resolution  # in meters per pixel
